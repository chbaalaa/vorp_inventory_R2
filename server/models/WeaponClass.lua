...
local Weapon <const> = LIB.Class:Create({
	...
	set = {
		...
		-- when we reload
		-- FIX: persist the weapon's clip ammo to the DB every time it changes.
		-- Previously this only updated the in-memory table, so ammo loaded into
		-- a weapon via reload was lost on relog unless CONFIG.MANUAL_WEAPON_RELOAD
		-- was true (the only thing that saved it was a periodic loop gated behind
		-- that config flag). Saving it here makes it reliable regardless of config
		-- and regardless of whether the player disconnects between save ticks.
		...
		-- FIX: also persist when ammo is consumed by firing, so the weapon's
		-- saved ammo count never drifts higher than what the player actually has.
