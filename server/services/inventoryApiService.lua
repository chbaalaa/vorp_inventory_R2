...
local function NotifyItem(source, itemName, amount, isAdd, icon)
	local title = isAdd and (LANG.WebHookLang?.added or "ได้รับ!") or (LANG.WebHookLang?.removed or "ลบออก!")
...
local function NotifyWeapon(source, weaponName, weaponLabel, isAdd)
	local title = isAdd and (LANG.WebHookLang?.weaponReceived or "ได้รับอาวุธ!") or (LANG.WebHookLang?.weaponRemoved or "ลบอาวุธออก!")
...
local function NotifyMoney(source, amount, isAdd)
	local title = isAdd and (LANG.WebHookLang?.moneyReceived or "ได้รับเงิน!") or (LANG.WebHookLang?.moneyRemoved or "ลบเงินออก!")
	local prefix = isAdd and "+" or "-"
...
local function NotifyGold(source, amount, isAdd)
	local title = isAdd and (LANG.WebHookLang?.goldReceived or "ได้รับทอง!") or (LANG.WebHookLang?.goldRemoved or "ลบทองออก!")
	local prefix = isAdd and "+" or "-"
...
local function NotifyAmmo(source, ammoType, ammoLabel, amount, isAdd)
	local title = isAdd and (LANG.WebHookLang?.ammoReceived or "ได้รับกระสุน!") or (LANG.WebHookLang?.ammoUsed or "ลบกระสุนออก!")
	local prefix = isAdd and "+" or "-"
	...
	MAIN = {
		...
		CAN_CARRY_ITEM_AMOUNT = function(source, _, cb)
			...
			local function cancarryammount()
				...
				local capacity <const> = tonumber(character.invCapacity) or 0
				if capacity == -1 then return true end
				return totalAmount + totalAmountWeapons <= capacity
			end
		...
		CAN_CARRY_ITEM = function(source, itemName, amount, cb)
			...
			local itemLimit <const> = tonumber(svItem.limit) or 0
			local itemWeight <const> = tonumber(svItem.weight) or 0
			local ignoresStackLimit <const> = itemLimit == -1
			local canCarryStack <const> = ignoresStackLimit or not exceedsItemLimit(character.identifier, itemLimit)

			if canCarryStack then
				canCarry = not exceedsInvLimit(character.identifier, character.charIdentifier, character.invCapacity, itemWeight)
			end
			...
			if item then
				...
				if item:getMaxDegradation() == 0 then
					...
					if result and doesMetadataExist then
						...
						if not allow then NotifyItem(_source, (item:getMetadata() or {}).label or item:getLabel(), amount, true, name) end
						return respond(cb, true)
					end
					...
					if itemNoMetadata then
						...
						if not allow then NotifyItem(_source, (itemNoMetadata:getMetadata() or {}).label or itemNoMetadata:getLabel(), amount, true, name) end
						return respond(cb, true)
					end
				end
			end
			...
			DB_SERVICE.CREATE.ITEM(charIdentifier, svItem:getId(), amount, metadata or {}, name, isExpired,
				svItem:getDurability(), function(craftedItem)
					...
					if not allow then
						...
						NotifyItem(_source, (newItem:getMetadata() or {}).label or newItem:getLabel(), amount, true, newItem:getName())
					end
		...
		SUB_ITEM_BY_ID = function(source, id, cb, allow, amount)
			...
			if not allow then
				...
				NotifyItem(_source, itemMetadata?.label or item:getLabel(), amount, false, itemName)
			end
			return respond(cb, true)
		end,
		...
		SUB_ITEM = function(source, name, amount, metadata, cb, allow, percentage)
			...
			if metadata then
				...
				if not allow then
					...
					TriggerEvent("vorp_inventory:Server:OnItemRemoved", data, _source)
					NotifyItem(_source, itemMetadata?.label or itemFound:getLabel(), amount, false, itemName)
				end
				return respond(cb, true)
			end
			...
			if exactMatchItem then
				...
				if not allow then
					local data <const> = { name = itemName, count = amount, metadata = itemMetadata }
					TriggerEvent("vorp_inventory:Server:OnItemRemoved", data, _source)
					NotifyItem(_source, itemMetadata?.label or exactMatchItem:getLabel(), amount, false, itemName)
				end
			else
				...
				for _, value in ipairs(itemsToRemove) do
					...
					if not allow then
						...
						TriggerEvent("vorp_inventory:Server:OnItemRemoved", data, _source)
						NotifyItem(_source, itemMetadata?.label or item:getLabel(), removeCount, false, itemName)
					end
				end
			end
			return respond(cb, true)
		...
		REMOVE_WEAPON_BULLETS = function(source, weaponId, bulletType, amount, cb)
			...
			if userWeapons:getPropietary() == identifier then
				...
				NotifyAmmo(_source, bulletType, nil, amount, false)
				return respond(cb, true)
			end
		...
		DELETE_WEAPON = function(source, weaponId, cb)
			...
			local weaponLabel = weapon:getCustomLabel() or weapon:getLabel()
			local shouldNotify = weapon:getPropietary() and weapon:getPropietary() ~= ''
			...
			if shouldNotify then
				NotifyWeapon(_source, weaponName, weaponLabel, false)
			end
		...
		ADD_WEAPON = function(source, wepname, ammos, _, comps, cb, wepId, customSerial, customLabel, customDesc, setUsed)
			...
			DB_SERVICE.ASYNC.INSERT(query, params, function(result)
				...
				NotifyWeapon(_source, name, label, true)
				if setUsed then
			...
			DB_SERVICE.ASYNC.UPDATE(query, params, function()
				if _target and _target > 0 then
					if CORE.getUser(_target) then
						...
						NotifyWeapon(_target, weaponName, customLabel or label, false)
					end
				end
				TriggerClientEvent("vorpInventory:receiveWeapon", _source, weaponId, weaponPropietary, weaponName,
					...
					weaponStatus)
				NotifyWeapon(_source, weaponName, customLabel or label, true)
			end)
		...
		REMOVE_WEAPON = function(source, weaponId, cb)
			...
			local weaponName = userWeapons:getName()
			local weaponLabel = userWeapons:getCustomLabel() or userWeapons:getLabel()
