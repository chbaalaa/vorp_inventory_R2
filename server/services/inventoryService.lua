...
local function HawkNotify(source, title, message, icon, duration, notifyType)
	if not source or source == 0 then return end
	icon = icon or "icon"
	duration = duration or 3000
	notifyType = notifyType or "info"
	TriggerClientEvent("hawk_notify", source, title, message, icon, duration, "right-bottom", notifyType)
end

local function NotifyItem(source, itemName, amount, isAdd, icon)
	local title = isAdd and (LANG.WebHookLang?.added or "ได้รับไอเทม!") or (LANG.WebHookLang?.removed or "เสียไอเทม!")
	local prefix = isAdd and "+" or "-"
	local notifyType = isAdd and "success" or "error"
	HawkNotify(source, title, prefix .. " " .. tostring(amount or 1) .. " " .. tostring(itemName or ""), icon or itemName, 3000, notifyType)
end

local function NotifyWeapon(source, weaponName, weaponLabel, isAdd)
	local title = isAdd and (LANG.WebHookLang?.weaponReceived or "ได้รับไอเทม!") or (LANG.WebHookLang?.weaponRemoved or "เสียไอเทม!")
	local icon = weaponName or "weapon"
	if weaponName and string.lower(weaponName) == "weapon_revolver_lemat" then
		icon = "weapon_revolver_doubleaction"
	end
	HawkNotify(source, title, tostring(weaponLabel or weaponName or ""), icon, 4000, isAdd and "success" or "error")
end

local function NotifyMoney(source, amount, isAdd)
	local title = isAdd and (LANG.WebHookLang?.moneyReceived or "ได้รับเงิน!") or (LANG.WebHookLang?.moneyRemoved or "เสียเงิน!")
	local prefix = isAdd and "+" or "-"
	HawkNotify(source, title, prefix .. " $" .. tostring(amount or 0), "money", 3000, isAdd and "success" or "error")
end

local function NotifyGold(source, amount, isAdd)
	local title = isAdd and (LANG.WebHookLang?.goldReceived or "ได้รับทอง!") or (LANG.WebHookLang?.goldRemoved or "เสียทอง!")
	local prefix = isAdd and "+" or "-"
	HawkNotify(source, title, prefix .. " " .. tostring(amount or 0) .. " Gold", "goldbar", 3000, isAdd and "success" or "error")
end
...
local function NotifyAmmo(source, ammoType, ammoLabel, amount, isAdd)
	local title = isAdd and (LANG.WebHookLang?.ammoReceived or "ได้รับกระสุน!") or (LANG.WebHookLang?.ammoUsed or "เสียกระสุน!")
	local prefix = isAdd and "+" or "-"
	local displayName = ammoLabel
	if not displayName and ammoType and SHARED_DATA and SHARED_DATA.AMMO_LABEL then
		displayName = SHARED_DATA.AMMO_LABEL[ammoType]
	end
	displayName = displayName or ammoType or "Ammo"
	HawkNotify(source, title, prefix .. " " .. tostring(amount or 0) .. " " .. tostring(displayName), GetAmmoNotifyIcon(ammoType), 3000, isAdd and "success" or "error")
end

local function PlayPickupAnim(source)
	local pickupAnim = nil
	if CONFIG.PICKUPS and CONFIG.PICKUPS.ANIMATIONS then
		pickupAnim = CONFIG.PICKUPS.ANIMATIONS.PICKUP
	end
	TriggerClientEvent("vorpInventory:playerPickUpAnim", source, pickupAnim)
end
...
local function PlayGiveSuccessAnimation(source, target)
	if not source or not target then return end
	TriggerClientEvent("vorpInventory:playGiveAnim", source, target)
	TriggerClientEvent("vorpInventory:playReceiveAnim", target, source)
end
...
local InventoryService <const> = {
	GIVE = {
		...
		MONEY = function(target, amount)
			...
			NotifyMoney(_source, amount, false)
			NotifyMoney(target, amount, true)
			PlayGiveSuccessAnimation(_source, target)
		...
		GOLD = function(target, amount)
			...
			NotifyGold(_source, amount, false)
			NotifyGold(target, amount, true)
			PlayGiveSuccessAnimation(_source, target)
		...
		WEAPON_TARGET = function(target, weaponId, source)
			...
			PlayGiveSuccessAnimation(source, target)
		...
		ITEM = function(itemId, amount, target)
			...
			local function updateClient(addedItem)
				...
				local label = itemMetadata?.label or (svItem:getMetadata() or {}).label or svItem:getLabel()
				NotifyItem(_source, label, amount, false, itemName)
				NotifyItem(target, label, amount, true, itemName)
				PlayGiveSuccessAnimation(_source, target)
			end
		...
		AMMO = function(ammotype, amount, target, maxcount)
			...
			NotifyAmmo(_source, ammotype, nil, amount, false)
			NotifyAmmo(target, ammotype, nil, amount, true)
			PlayGiveSuccessAnimation(_source, target)
			SV_UTILS.PROCESS.REMOVE_USER(_source)
		end,
	...
	PICKUP = {
			...
			if pickup.isItem == 1 then
				...
				INVENTORY_SERVICE.ITEM.ADD(_source, "default", pickup.name, pickup.amount, pickup.metadata, info, function(_item)
					if _item and PICKUPS.ITEMS[data.uid] then
						...
						PlayPickupAnim(_source)
						NotifyItem(_source, (_item:getMetadata() or {}).label or _item:getLabel(), pickup.amount, true, _item:getName())
						local charname <const>    = character.firstname .. ' ' .. character.lastname
		...
		MONEY = function(data)
			...
			NotifyMoney(_source, money.amount, true)
		...
		GOLD = function(data)
			...
			NotifyGold(_source, picup.amount, true)
			SV_UTILS.DISCORD_LOG({
				source = _source,
				name = CONFIG.LOGS.webhookname,
	...
	DROP = {
		...
		SHARE_WEAPON = function(source, callback, data)
			...
			if CONFIG.PICKUPS.DELETE_ON_DROP then
				...
				-- FIX: log the drop to Discord here too. Previously this branch
				-- returned immediately and skipped the webhook code below entirely,
				-- so weapons dropped with DELETE_ON_DROP = true were never logged.
				do
					local serialNumber = weapon:getSerialNumber()
					local desc = weapon:getCustomDesc()
					...
					local dropDescription <const> = "**" .. LANG.WebHookLang.Weapontype .. ":** `" .. weapon:getName() .. "`\n**" .. LANG.WebHookLang.charname .. ":** `" .. charname .. "`\n**" .. LANG.WebHookLang.serialnumber .. "** ` " .. serialNumber .. " ` \n **" .. LANG.WebHookLang.Desc .. "** `" .. desc .. "` \n **" .. LANG.WebHookLang.Steamname .. "** `" .. steamname .. "`"
					SV_UTILS.DISCORD_LOG({
						source = source,
						name = CONFIG.LOGS.webhookname,
			...
			NotifyWeapon(source, weapon:getName(), weapon:getCustomLabel() or weapon:getLabel(), false)
		...
		SHARE_ITEM = function(source, callback, data)
			...
			-- NOTE: ITEM.REMOVE only updates the server side (memory + DB).
			-- Without this, the client never learns the item was removed, so its
			-- icon keeps showing in the bag NUI until the inventory is reopened.
			TriggerClientEvent("vorpCoreClient:subItem", source, data.id, item:getCount())

			if CONFIG.PICKUPS.DELETE_ON_DROP then
				-- FIX: log the drop to Discord here too. Previously this branch
				-- returned immediately and skipped the webhook code below entirely,
				-- so items dropped with DELETE_ON_DROP = true were never logged.
				do
					local dropCharname <const> = character.firstname .. ' ' .. character.lastname
					local dropSteamname <const> = GetPlayerName(source) or ""
					local dropDescription <const> = "**" .. LANG.WebHookLang.amount .. "** `" .. data.amount .. "`\n **" .. LANG.WebHookLang.itemDrop .. "**: `" .. data.name .. "`" .. "\n**" .. LANG.WebHookLang.charname .. ":** `" .. dropCharname .. "`\n**" .. LANG.WebHookLang.Steamname .. "** `" .. dropSteamname .. "`"
					SV_UTILS.DISCORD_LOG({
						source = source,
						name = CONFIG.LOGS.webhookname,
			...
			NotifyItem(source, (item:getMetadata() or {}).label or item:getLabel(), data.amount, false, data.name)
		...
		SHARE_MONEY = function(source, callback, data)
			...
			NotifyMoney(source, data.amount, false)
			...
			-- FIX: this function had no Discord webhook log at all. Added one,
			-- mirroring the PICKUP.MONEY log style, so dropped money is logged
			-- regardless of CONFIG.PICKUPS settings.
			do
				local dropCharname <const> = character.firstname .. ' ' .. character.lastname
				local dropSteamname <const> = GetPlayerName(source) or ""
				local dropDescription <const> = "**" .. LANG.WebHookLang.money .. ":** `" .. data.amount .. "` `$` \n**" .. LANG.WebHookLang.charname .. ":** `" .. dropCharname .. "`\n**" .. LANG.WebHookLang.Steamname .. "** `" .. dropSteamname .. "`\n"
				SV_UTILS.DISCORD_LOG({
					source = source,
					name = CONFIG.LOGS.webhookname,
					title = (LANG.WebHookLang and LANG.WebHookLang.dropedmoney) or "ทิ้งเงิน (Dropped Money)",
					description = dropDescription,
					webhook = CONFIG.LOGS.webhook,
					color = CONFIG.LOGS.colorDropMoney,
				})
			end
		...
		SHARE_GOLD = function(source, callback, data)
			...
			NotifyGold(source, data.amount, false)
			local uid <const> = SV_UTILS.GENERATE_UNIQUE_ID()
			TriggerClientEvent("vorpInventory:shareGoldPickupClient", -1, data.handle, data.amount, data.position, uid, 1, data.rotation)

			-- FIX: this function had no Discord webhook log at all. Added one,
			-- mirroring the PICKUP.GOLD log style, so dropped gold is logged
			-- regardless of CONFIG.PICKUPS settings.
			do
				local dropCharname <const> = character.firstname .. ' ' .. character.lastname
				local dropSteamname <const> = GetPlayerName(source) or ""
				local dropDescription <const> = "**" .. LANG.WebHookLang.gold .. ":** `" .. data.amount .. "` \n**" .. LANG.WebHookLang.charname .. ":** `" .. dropCharname .. "`\n**" .. LANG.WebHookLang.Steamname .. "** `" .. dropSteamname .. "`\n"
				SV_UTILS.DISCORD_LOG({
					source = source,
					name = CONFIG.LOGS.webhookname,
					title = (LANG.WebHookLang and LANG.WebHookLang.droppedgold) or "ทิ้งทอง (Dropped Gold)",
					description = dropDescription,
					webhook = CONFIG.LOGS.webhook,
					color = CONFIG.LOGS.colorDropGold,
				})
		...
		SHARE_ROLL = function(source, callback, data)
			...
			-- FIX: this function had no Discord webhook log at all. Added one,
			-- mirroring the PICKUP.ROLL log style, so dropped roll is logged
			-- regardless of CONFIG.PICKUPS settings. No dedicated "roll" color
			-- exists in CONFIG.LOGS, so colorDropGold is reused here.
			do
				local dropCharname <const> = character.firstname .. ' ' .. character.lastname
				local dropSteamname <const> = GetPlayerName(source) or ""
				local rolLabel <const> = (LANG.WebHookLang and LANG.WebHookLang.roll) or "Roll"
				local dropDescription <const> = "**" .. rolLabel .. ":** `" .. data.amount .. "` \n**" .. LANG.WebHookLang.charname .. ":** `" .. dropCharname .. "`\n**" .. LANG.WebHookLang.Steamname .. "** `" .. dropSteamname .. "`\n"
				SV_UTILS.DISCORD_LOG({
					source = source,
					name = CONFIG.LOGS.webhookname,
					title = (LANG.WebHookLang and LANG.WebHookLang.droppedroll) or "ทิ้ง Roll (Dropped Roll)",
					description = dropDescription,
					webhook = CONFIG.LOGS.webhook,
					color = CONFIG.LOGS.colorDropGold,
				})
	...
	WEAPON = {
		...
		ADD_BULLET = function(ammotype, amount, weaponid)
			...
			NotifyAmmo(_source, ammotype, nil, amount, true)
		end,
	...
	AMMO = {
		-- FIX: previously, if the client's ammo payload contained even one
		-- ammo type the server didn't already have a key for (e.g. the very
		-- first time this character ever carried that ammo type), this whole
		-- function would `return` immediately and silently drop the sync for
		-- *every* ammo type in that call, not just the unknown one. That's why
		-- in auto (CONFIG.MANUAL_WEAPON_RELOAD = false) mode the belt could
		-- show correct numbers client-side (it reads the ped directly) while
		-- the server-side cache, and therefore the DB, never got updated.
		-- Now we initialize unknown types instead of aborting, and skip only
		-- the offending entry on a cheat-check failure instead of the whole
		-- batch.
		UPDATE = function(ammoinfo)
			...
			for ammoType, amount in pairs(ammoinfo.ammo) do
				if not userAmmoData.ammo[ammoType] then
					userAmmoData.ammo[ammoType] = 0
				end

				-- client can never have more than the server
				if amount > userAmmoData.ammo[ammoType] then
					print("amount is greater than what we have :", amount, "we have :", userAmmoData.ammo[ammoType], "ammo type :", ammoType, "possible cheat!!", GetPlayerName(_source))
				else
					userAmmoData.ammo[ammoType] = math.max(0, math.min(amount, SHARED_DATA.MAX_AMMO_BELT[ammoType] or amount))
				end
			end
		end,
	...
	SECONDARY = {
		...
		MOVE_TO = function(obj)
			...
			if item.type == "item_weapon" then
				...
				NotifyWeapon(_source, weapon:getName(), weapon:getCustomLabel() or weapon:getLabel(), false)
				...
				INVENTORY_SERVICE.ITEM.ADD(_source, invId, item.name, amount, item.metadata, info, function(itemAdded)
					...
					NotifyItem(_source, item.metadata?.label or item.label, amount, false, item.name)
		...
		TAKE_FROM = function(obj)
			...
			if item.type == "item_weapon" then
				...
				NotifyWeapon(_source, name, custom or label, true)
				...
				INVENTORY_SERVICE.ITEM.ADD(_source, "default", item.name, amount, item.metadata, info, function(itemAdded)
					...
					NotifyItem(_source, (itemAdded:getMetadata() or {}).label or itemAdded:getLabel(), amount, true, itemAdded:getName())
						...
						INVENTORY_API.MAIN.SUB_ITEM(_source, item.name, amount, item.metadata, function(result2)
							if result2 then
								SetTimeout(400, function()
									...
									NotifyItem(_source, metadataLabel, amount, false, item.name)
									NotifyItem(target, metadataLabel, amount, true, item.name)
									SV_UTILS.PROCESS.REMOVE_USER(_source)
								end)
							else
								SV_UTILS.PROCESS.REMOVE_USER(_source)
					...
					if result then
						INVENTORY_API.MAIN.SUB_ITEM(target, item.name, amount, item.metadata, function(result2)
							if result2 then
								...
								INVENTORY_SERVICE.LOGS.DISCORD(invId, item.name, amount, sourceName, "Take")
								local metadataLabel = item.metadata?.label or item.label
								NotifyItem(_source, metadataLabel, amount, true, item.name)
								NotifyItem(target, metadataLabel, amount, false, item.name)
							end
							SV_UTILS.PROCESS.REMOVE_USER(_source)
						end, true)
					else
...
CreateThread(function()
	...
	LIB.SetInterval(function()
		...
		-- FIX: this used to only run when CONFIG.MANUAL_WEAPON_RELOAD was true,
		-- meaning any ammo loaded into a weapon's clip (via addAmmoToClip, e.g.
		-- from the RELOADED event) was never periodically flushed to the DB when
		-- the config flag was false. addAmmoToClip/subAmmoFromClip now persist
		-- immediately on their own (see WeaponClass.lua), but we keep this loop
		-- running unconditionally as a safety net / catch-all in case of missed
		-- writes, regardless of the reload mode in use.
