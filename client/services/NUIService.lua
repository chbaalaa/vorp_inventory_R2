...
-- GIVE/RECEIVE ITEM ANIMATION
local giveAnimDict    = "script_common@mth_generic_enters@give_item_satchel@rhand@lleg_forward"
local giveAnimName    = "enter_rf"
local receiveAnimDict = "script_common@mth_generic_enters@give_item_satchel@rhand@generic@step_forward"
local receiveAnimName = "enter_lf"

local function PlayGiveItemAnim(targetPed)
	...
	if not HasAnimDictLoaded(giveAnimDict) then
		RequestAnimDict(giveAnimDict)
		local timeout = 0
		repeat
			Wait(0)
			timeout = timeout + 1
		until HasAnimDictLoaded(giveAnimDict) or timeout > 100
	end

	if HasAnimDictLoaded(giveAnimDict) then
		TaskPlayAnim(playerPed, giveAnimDict, giveAnimName, 2.0, 2.0, -1, 0, 0, false, false, false)
		Wait(2000)
		ClearPedTasks(playerPed, true, true)
		RemoveAnimDict(giveAnimDict)
	end
end

local function PlayReceiveItemAnim(giverPed)
	local playerPed = PlayerPedId()

	if giverPed and DoesEntityExist(giverPed) then
		TaskTurnPedToFaceEntity(playerPed, giverPed, 1500)
		Wait(1500)
	end

	if not HasAnimDictLoaded(receiveAnimDict) then
		RequestAnimDict(receiveAnimDict)
		local timeout = 0
		repeat
			Wait(0)
			timeout = timeout + 1
		until HasAnimDictLoaded(receiveAnimDict) or timeout > 100
	end

	if HasAnimDictLoaded(receiveAnimDict) then
		TaskPlayAnim(playerPed, receiveAnimDict, receiveAnimName, 2.0, 2.0, -1, 0, 0, false, false, false)
		Wait(2000)
		ClearPedTasks(playerPed, true, true)
		RemoveAnimDict(receiveAnimDict)
	end
end

local function GetSirevlcHorseId()
	if GetResourceState("sirevlc_horses") ~= "started" then return nil end

	local ok, elements = pcall(function()
		return exports["sirevlc_horses"]:GET_PLAYER_HORSE_ELEMENTS()
	end)

	if not ok or type(elements) ~= "table" then return nil end
	return elements.ID or elements.id or elements.HORSE_ID or elements.horseId or elements.HorseId or elements.horse_id or elements.DATABASE_ID or elements.databaseId
end

local function HasSirevlcHorse()
	if GetResourceState("sirevlc_horses") ~= "started" then return false end

	local ok, horse = pcall(function()
		return exports["sirevlc_horses"]:GET_PLAYER_HORSE()
	end)

	return ok and horse and horse ~= 0 and horse ~= false
end

local function CanUseSaddleInventory()
	return GetMount(CACHE.Ped) > 0
end

local function TryOpenSirevlcSaddleInventory(mount)
	if GetResourceState("sirevlc_horses") ~= "started" then return false end
	if not HasSirevlcHorse() then return false end

	local horseId = GetSirevlcHorseId()
	local netId = mount and NetworkGetNetworkIdFromEntity(mount) or nil
	NUI_SERVICE.INVENTORY.CLOSE()
	TriggerServerEvent("vorp_inventory:Server:SirevlcSaddleOpen", horseId, netId)
	return true
end

RegisterNetEvent("vorpInventory:playGiveAnim")
AddEventHandler("vorpInventory:playGiveAnim", function(targetServerId)
	CreateThread(function()
		...
		PlayGiveItemAnim(targetPed)
	end)
end)

RegisterNetEvent("vorpInventory:playReceiveAnim")
AddEventHandler("vorpInventory:playReceiveAnim", function(giverServerId)
	CreateThread(function()
		local giverPed = nil

		if giverServerId then
			for _, playerId in ipairs(GetActivePlayers()) do
				if GetPlayerServerId(playerId) == giverServerId then
					giverPed = GetPlayerPed(playerId)
					break
				end
			end
		end

		PlayReceiveItemAnim(giverPed)
	end)
end)
...
local nuiService = {
	...
	INVENTORY = {
		...
		OPEN = function()
			...
			SendNUIMessage({
				...
				IsOnMount = CanUseSaddleInventory(),
			})
			IS_INV_OPEN = true
			NUI_SERVICE.INVENTORY.GET_LOAD()
		end,
	...
	SHARED = {
		...
		PLACE = function(obj)
			...
			if CONFIG.INVENTORY_UI.ADVANCED_DROP == false then
				return CORE.NotifyRightTip(LANG.cantdrophere, 5000)
			end
		...
		SEND_LANG = function()
			SendNUIMessage({
				...
				config = {
					...
					EnableAdvancedDrop = CONFIG.INVENTORY_UI.ADVANCED_DROP ~= false,
					EnableDragDropDiscard = CONFIG.INVENTORY_UI.ENABLE_DRAG_DROP_DISCARD == true,
	...
	SADDLE = {
		OPEN = function(_, cb)
			...
			if mount <= 0 then
				return CORE.NotifyObjective("You are not on a horse", 5000)
			end

			if TryOpenSirevlcSaddleInventory(mount) then return end
