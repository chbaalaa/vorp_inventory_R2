-- true once GET_LOADOUT has finished actually giving weapons to the ped for this session.
-- ammoService.lua's RESTORE_AMMO_ON_JOIN waits on this before restoring belt ammo, instead of
-- guessing a fixed delay (see GET_LOADOUT below for where it flips true).
LOADOUT_READY = false
...
local inventory <const> = {
	...
	ON_SELECTED_CHARACTER = function()
		LOADOUT_READY = false -- reset for character switch without a resource restart
	...
	GET_LOADOUT = function(loadout)
		...
		LOADOUT_READY = true -- weapons are on the ped now, safe to restore belt ammo
	...
	ASK_TO_GIVE_ITEMS = function(cb, data)
		...
		-- CONFIG.GIVE_ITEMS_CONFIRMATION_ENABLED: true (default) = แจ้งเตือน + ต้องกด Accept/Deny ยืนยันก่อนรับของ
		--                                          false          = รับของทันทีโดยไม่ต้องยืนยัน ไม่ต้องแจ้งเตือน
		if CONFIG.GIVE_ITEMS_CONFIRMATION_ENABLED == false then
			return cb(true)
		end

		CORE.NotifyObjective(LANG.someoneWantsToGiveYouSomething, 5000)

		local timeoutMs <const> = 30000 -- ระยะเวลารอยืนยันแลกเปลี่ยน
