...
local LAST_IMMEDIATE_AMMO_SYNC = 0


local Ammo <const> = {
    -- applies belt ammo straight to the ped, no Wait/yield.
    -- must run atomically with whatever set PLAYER_AMMO_INFO.ammo (see UPDATE_AMMO) so the
    -- AMMO SAVING THREAD below never gets a chance to read a ped ammo value that hasn't been
    -- written yet - that race is what silently cancels ammo refills.
    -- only safe to call once the ped's weapons already exist (mid-game). for restoring ammo
    -- on join, use RESTORE_AMMO_ON_JOIN instead.
    ...
    -- called once on character join. GET_LOADOUT (InventoryService.lua) gives weapons to the ped
    -- with 0 ammo and only sets LOADOUT_READY once that finishes. Setting belt ammo before that
    -- point gets silently reset when GiveWeaponToPed runs, which is what caused ammo to
    -- disappear from the gun on relog with a short fixed Wait(). We poll for LOADOUT_READY instead
    -- of guessing a duration, with a timeout as a fallback in case loadout never fires.
    RESTORE_AMMO_ON_JOIN = function(ammoData)
        if not ammoData then return end
        CreateThread(function()
            local timer <const> = GetGameTimer()
            repeat
                Wait(50)
            until LOADOUT_READY or (GetGameTimer() - timer) > 15000
    ...
    UPDATE_AMMO = function(ammoData, setAmmoToPed, isSource, removeAll)
        ...
        if setAmmoToPed then
            if not CONFIG.MANUAL_WEAPON_RELOAD then
                -- runtime refill (already in-session, weapons already equipped): apply
                -- immediately and atomically, no Wait, so the saving thread below can't race it
                AMMO_SERVICE.ADD_AMMO_TO_PED(ammoData.ammo)
            end
        end
    ...
    LOAD_AMMO = function(ammoData)
        ...
        if not CONFIG.MANUAL_WEAPON_RELOAD then
            -- join flow: wait for the loadout/weapons to actually be on the ped first
            AMMO_SERVICE.RESTORE_AMMO_ON_JOIN(PLAYER_AMMO_INFO.ammo)
        end
...
if not CONFIG.MANUAL_WEAPON_RELOAD then
    local function syncAmmoChange(ammoType, ammoQty, previousQty)
        UPDATE_AMMO_CACHE[ammoType] = ammoQty
        PLAYER_AMMO_INFO.ammo[ammoType] = ammoQty
        SendNUIMessage({ action = "updateammo", ammo = PLAYER_AMMO_INFO.ammo })

        if canUpdateAmmo and ammoQty < previousQty then
            local now <const> = GetGameTimer()
            if now - LAST_IMMEDIATE_AMMO_SYNC > 100 then
                LAST_IMMEDIATE_AMMO_SYNC = now
                ...
                if (isArmed or isThrownGroup or isPetrol) and not ismelee then
                    ...
                    for ammo_type, value in pairs(PLAYER_AMMO_INFO.ammo) do
                        ...
                        if value ~= ammoQty then
                            syncAmmoChange(ammo_type, ammoQty, value)
                        end
                    end
                end
            end
