...
local pickups <const>       = {
	...
	PLAY_ANIM = function(animation)
		...
		if now - LastPickupAnim < 4000 then return end
		LastPickupAnim = now

		local pickupAnimation = animation
		if type(pickupAnimation) ~= "table" or pickupAnimation.AnimDict == "amb_player@world_player_chore@bucket_put_down@male_a@base" then
			pickupAnimation = CONFIG.PICKUPS.ANIMATIONS.PICKUP
		end

		UTILS.PLAY_ANIM(pickupAnimation)

		local sound <const> = CONFIG.SFX and CONFIG.SFX.PICK_UP
		if sound and sound.ENABLE then
					...
					if pickup.entityId == WORLD_PICKUPS[key].entityId then
						if not pressed then
							...
							if isAnyPlayerNear() == 0 then
								...
								PICKUP_SERVICE.PLAY_ANIM(CONFIG.PICKUPS.ANIMATIONS.PICKUP)
