...
local function VorpInventoryHawkNotify(message, duration)
    ...
    if string.find(lower, "received", 1, true) or string.find(lower, "picked up", 1, true) or string.find(lower, "set to", 1, true) then
        notifyType = "success"
        title = "สำเร็จ"
    ...
    if GetResourceState("hawk_notify") == "started" then
        TriggerEvent("hawk_notify", title, text, icon, duration or 3000, "right-bottom", notifyType)
        return
    end
end

CORE.NotifyRightTip = VorpInventoryHawkNotify
CORE.NotifyObjective = VorpInventoryHawkNotify
        ...
        while true do
            ...
            if not IS_INV_OPEN then
                ...
                if IsControlPressed(0, CONFIG.HOTBAR.HOLD_KEY) then
                    ...
                    -- ตอนกด ALT ค้าง ให้ปิด control การสลับ/เลือกอาวุธด่วนทั้งหมด (ปุ่ม 1-8)
                    -- พร้อมกันทั้งชุด ไม่ว่าผู้เล่นจะ remap ปุ่มไหนไปช่องไหนก็ตาม
                    -- เพื่อไม่ให้อาวุธออกมาซ้อนกับการใช้ไอเทมจาก hotbar
                    for _, control in ipairs(CONFIG.HOTBAR.BLOCK_WEAPON_CONTROLS) do
                        DisableControlAction(0, control, true)
                    end

                    -- ตรวจจับว่ากดเลขไหนด้วย raw key (VK code) ตามปุ่มคีย์บอร์ดจริงเสมอ
                    -- ไม่ต้อง disable raw key เพราะ control ที่ต้องบล็อกถูกปิดแยกไว้ข้างบนแล้ว
                    for i = 1, 5 do
                        if IsRawKeyPressed(CONFIG.HOTBAR.SLOT_KEYS_RAW[i]) then
