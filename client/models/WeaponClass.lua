...
local Weapon <const> = LIB.Class:Create({
	...
	set = {
		...
		addAmmoToClip        = function(self, type, amount, skipTrigger)
			-- BUGFIX: previously, when self.ammo[type] was nil (e.g. right after
			-- cleanAmmoFromClip/cleanAllAmmoFromClip wiped it on empty/unequip), this
			-- function first set self.ammo[type] = amount, then IMMEDIATELY added `amount`
			-- again in the next block (self.ammo[type] + amount), doubling the ammo that
			-- gets loaded. That's the "gets extra ammo it shouldn't have" bug - it wasn't
			-- actually old fired ammo coming back, it was this amount being counted twice.
			local current <const> = self.ammo[type] or 0
			if self.defaultClipSize > 0 then
				self.ammo[type] = math.min(current + amount, self.defaultClipSize)
			else
				self.ammo[type] = current + amount
			end
